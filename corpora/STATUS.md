...
|---|---:|---:|---|
| `mixed-app-text` | exact | `60/61 exact` | remaining Chrome-only `710px` miss is SHY / extractor-sensitive; Safari is exact there again in height/line count |
...
| `ja-rashomon` | Japanese | exact | exact | `5/9 exact` | `55/61 exact` | real Japanese canary; remaining field is mostly opening-quote / punctuation compression plus a few one-line edge fits |
| `ko-unsu-joh-eun-nal` | Korean | exact | not recently rerun | `9/9 exact` | `61/61 exact` | Korean coarse corpus is clean |
| `zh-guxiang` | Chinese | exact at `600 / 800`, `+64px` at `300` | exact | `6/9 exact` | `54/61 exact` | second Chinese canary; same Chrome-positive / Safari-clean split as `祝福`, but slightly healthier overall |
| `zh-zhufu` | Chinese | exact at `600 / 800`, `+32px` at `300` | exact | `7/9 exact` | `42/61 exact` | real Chinese canary; broad positive one-line field in Chrome, exact Safari anchors |
| `th-nithan-vetal-story-1` | Thai | exact | exact | `9/9 exact` | `59/61 exact` | two remaining coarse one-line misses |
| `th-nithan-vetal-story-7` | Thai | exact | exact | `9/9 exact` | `60/61 exact` | second Thai canary stays healthy |
| `km-prachum-reuang-preng-khmer-volume-7-stories-1-10` | Khmer | exact | exact | `9/9 exact` | `61/61 exact` | full `step=10` is slower; sampled check is the preferred first pass |
| `my-cunning-heron-teacher` | Myanmar | exact | exact | `9/9 exact` | `56/61 exact` | real residual Myanmar canary; quote/follower and phrase-break classes remain |
...
| `hi-eidgah` | Hindi | exact | exact | `9/9 exact` | `61/61 exact` | Hindi coarse corpus is clean |
| `ar-risalat-al-ghufran-part-1` | Arabic | exact | exact at key sentinels | `9/9 exact` | `61/61 exact` | Arabic coarse corpus is clean; fine sweep still has a small positive one-line field |
| `ar-al-bukhala` | Arabic | exact | exact at anchors | `9/9 exact` | `61/61 exact` | large second Arabic canary; anchors are clean |
| `he-masaot-binyamin-metudela` | Hebrew | exact | exact | `9/9 exact` | `61/61 exact` | Hebrew coarse corpus is clean |
